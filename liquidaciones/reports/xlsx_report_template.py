            ...
            if exists_st:
                if exists_st.price_type == 'open':
                    ...
                    for line in settlement_id.settlements_line_ids:
                        ...
                        i += 1
                        sheet.write(i, 0, '', light_box)
                        sheet.write(i, 0, line.product_id.name, light_box)
                        sheet.write(i, 1, variant, light_box)
                        sheet.write(i, 2, line.box_rec, light_box)
                        sheet.write(i, 3, line.price_unit, light_box_currency)
                        sheet.write(i, 4, "=c{}*d{}".format(str(i+1), str(i+1)), light_box_currency)
                        sheet.write(i, 5, "", light_box_currency)
                        sheet.write(i, 6, "", light_box_currency)
                        sheet.write(i, 7, "", light_box_currency)
                        sheet.write(i, 8, "", light_box_currency)
                    ...
                    sheet.write(i+1, 3, "", light_box)
                    #sheet.write(i+1, 4, '', light_box)
                    sheet.write(i+1, 4, "=sum(e{}:e{})".format(str(inicio_i+1), str(i+1)), light_box_currency)
                    sheet.write(i+1, 2, "=sum(c{}:c{})".format(str(inicio_i+1), str(i+1)), light_box)
                    sheet.write(i+1, 5, settlement_id.freight_total, light_box_currency)
                    sheet.write(i+1, 6, settlement_id.aduana_total, light_box_currency)
...
class XlsxUtilityReport_firma(models.AbstractModel):    
    ...
    def generate_xlsx_report(self, workbook, data, objects, sign=True):
        ...
        for po in procesar_object:
            ...
            utility_per_qty = 0
            liquidaciones = []
            sheet = workbook.add_worksheet(_('{}'.format(po.lot)))
            sheet.set_landscape()
            sheet.fit_to_pages(1, 0)
            sheet.set_zoom(100)
            sheet.set_column(4, 0, 25)
            ...
            if exists_st:
                if exists_st.price_type == 'open':
                    ...
                    for col in range(12, 60):
                        for row in range(0, i):
                            sheet.write(row, col, '', clean_cell_format)
